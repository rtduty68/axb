...
def bridge(con,  uuid,  caller, callee, ringback, record):
    try:
        ...
        ret = con.execute("bridge", "{instant_ringback=true}[leg_timeout=8]sofia/external/13812341234@192.168.106.73:5019|sofia/external/13811334545@127.0.0.1:6060", uuid)
        if ret  == None: #连接已断开
            return "disconnected"
    except Exception,e:
